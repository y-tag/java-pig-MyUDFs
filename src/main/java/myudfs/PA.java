...
public class PA extends BinaryOnlineClassifier {
    ...
    public PA(List<PA> classiferList) {
        if (classiferList.size() == 0) {
            return;
        }

        for (int i = 0; i < classiferList.size(); i++) {
            if (classiferList.get(i).featureBit > this.featureBit) {
                this.featureBit = classiferList.get(i).featureBit;
                this.bitMask    = classiferList.get(i).bitMask;
            }
        }
        this.paType = classiferList.get(0).paType;
        this.etaCalculator = getEtaCalculator(this.paType);
        this.C = classiferList.get(0).C;

        int featureNum = 1 << this.featureBit;
        this.weightArray = new float[featureNum];

        for (int i = 0; i < classiferList.size(); i++) {
            for (int j = 0; j < classiferList.get(i).weightArray.length; j++) {
                this.weightArray[j] += classiferList.get(i).weightArray[j];
            }
        }

        for (int i = 0; i < featureNum; i++) {
            this.weightArray[i] /= classiferList.size();
        }
